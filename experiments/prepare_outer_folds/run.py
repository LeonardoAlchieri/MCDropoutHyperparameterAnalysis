...
def main():
    ...
    for task_num in tqdm(datasets_to_use, desc="Dataset progress"):
        ...
        if task_type == "regression":
            clf = KFold(n_splits=n_outer_folds, shuffle=True, random_state=random_seed)
        else:
            clf = StratifiedKFold(
                n_splits=n_outer_folds, shuffle=True, random_state=random_seed
            )
        train_idxs = {
            outer_fold: train_idx
            for outer_fold, (train_idx, _) in enumerate(clf.split(x, y))
        }
        test_idxs = {
            outer_fold: test_idx
            for outer_fold, (_, test_idx) in enumerate(clf.split(x, y))
        }
