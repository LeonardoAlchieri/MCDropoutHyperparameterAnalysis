...
def dimensionality_stats(X_train: np.ndarray, **kwargs) -> dict[str, float]:
	dimensionality = X_train.shape[1]
	pca = PCA(random_state=kwargs['random_seed'])
	pca.fit(X_train)
	exp_var = 0
	intrinsic_dim = 0
	...
	return dict(dimensionality=dimensionality, intrinsic_dim=intrinsic_dim, intrinsic_dim_ratio=intrinsic_dim_ratio, feature_noise=feature_noise, dist=dist)


def homogeneity_class_covariances(X_train: np.ndarray, y_train: np.ndarray, **kwargs) -> dict[str, float]:
	...
	return dict(levene_stat_avg=levene_stat_avg, levene_pval_avg=levene_pval_avg, levene_success_ratio=levene_success_ratio)


def feature_correlation_class(X_train: np.ndarray, y_train: np.ndarray, **kwargs) -> dict[str, float]:
...
#	fcc_std = np.std( rho )
	...
	return dict(fcc_mean=fcc_mean)


def normality_departure(X_train: np.ndarray, y_train: np.ndarray, **kwargs) -> dict[str, float]:
...
#	kurtosis_std = np.std( kurtosis_list )
	...
	return dict(skew_mean=skew_mean, kurtosis_mean=kurtosis_mean)


def information(X_train: np.ndarray, y_train: np.ndarray, **kwargs) -> dict[str, float]:
	
	mi = mutual_info_classif(X_train, y_train, random_state=kwargs['random_seed'])
	if sum(mi) == 0: 
		mi_mean = 0
	else:
...
#	mi_std = np.std( mi )
	...
	return dict(mi_mean=mi_mean)


def class_stats(y_train: np.ndarray, **kwargs) -> dict[str, float]:
	values, counts = np.unique(y_train, return_counts=True)
	pmax = np.max(counts)
	pmin = np.min(counts)
	imbalance_ratio = pmax / pmin
	return dict(imbalance_ratio=imbalance_ratio)
