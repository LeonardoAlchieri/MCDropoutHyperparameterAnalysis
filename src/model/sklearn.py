# Creating a custom MLPDropoutClassifier classifier
from sklearn.base import check_is_fitted, RegressorMixin, is_classifier
...
class BaseMultilayerPerceptronDropout(BaseMultilayerPerceptron):
    ...
    def _create_mask_for_layer(self, units: int, keep_probability: float) -> np.ndarray:
    ...
    def _create_dropout_masks(self, layer_units: list) -> list:
        dropout_masks = None
        ...
        if self.dropout != None:
            if 0 < self.dropout < 1:
                ...
                if self.dropout_input:
                    dropout_masks = [self._create_mask_for_layer(layer_units[0], keep_probability)]
                else:
                ...
                for units in layer_units[1:-1]:
                    ...
                    dropout_masks.append(self._create_mask_for_layer(units, keep_probability))
            else:
    ...
    def _forward_pass(self, activations, dropout_masks=None):
        ...
        if self.dropout_input:
            activations[0] = activations[0] * dropout_masks[0][None, :]
    ...
    def _forward_pass_fast(self, X, check_input=True, dropout_masks=None):
        ...
        for i in range(self.n_layers_ - 1):
            ...
            # Apply Dropout Mask (DROPOUT ADDITION)
            if i != self.n_layers_ - 2 and dropout_masks is not None:
                activation = activation * dropout_masks[i+1][None, :]
        output_activation = ACTIVATIONS[self.out_activation_]
...
class MLPDropoutClassifier(MLPClassifier):
    ...
    def __init__(
        ...
        *,
        ...
        mcdropout: bool, default=False
            If True, the model will use Monte Carlo Dropout during inference.
...
class MLPDropoutRegressor(BaseMultilayerPerceptronDropout, MLPRegressor):
